...
if ($in{'cmd'} =~ "setzfs") {
	$in{'confirm'} = "yes";
	if ($in{'set'} =~ "inherit" && $conf{'zfs_properties'}) { $cmd = "zfs inherit $in{'property'} $in{'zfs'}"; 
	} elsif ($conf{'zfs_properties'}) { $cmd =  "zfs set $in{'property'}=$in{'set'} $in{'zfs'}"; }
	...
	my $cmd = ($conf{'pool_properties'}) ? "zpool set $in{'property'}=$in{'set'} $in{'pool'}": undef;
	...
	my $cmd = (($in{'parent'}) && ($conf{'zfs_properties'})) ? cmd_create_zfs($in{'parent'}."/".$in{'zfs'}, \%options) : undef;
	$in{'confirm'} = "yes";
	...
	$in{'confirm'} = "yes";
	my $cmd =  ($conf{'zfs_properties'}) ? "zfs clone ".$in{'clone'}." ".$in{'parent'}.'/'.$in{'zfs'}." ".$opts : undef;
	...
	if (index($in{'zfs'}, '@') != -1) { 
		...
		#$in{'parent'} = undef;
		$cmd = ($conf{'snap_properties'}) ? "zfs rename ".$in{'force'}.$in{'recurse'}.$in{'zfs'}." ".$in{'parent'}.'@'.$in{'name'} : undef;
		...
		#$in{'name'} = $in{'parent'}.'/'.$in{'name'};
		#$in{'parent'} = undef;
		$cmd = ($conf{'zfs_properties'}) ? "zfs rename ".$in{'force'}.$in{'prnt'}.$in{'zfs'}." ".$in{'parent'}.'/'.$in{'name'} : undef; 
	}
	...
	my $cmd = $conf{'pool_properties'}) 
            ? cmd_create_zpool($in{'pool'}, $in{'vdev'}.$in{'devs'}, \%options, \%poolopts, $in{'force'}) 
            : undef;
	$in{'confirm'} = "yes";
	...
	$in{'confirm'} = "yes";
	my $cmd =  ($conf{'pool_properties'}) ? "zpool $in{'action'} $in{'pool'} $in{'vdev'}": undef;
	print ui_cmd("$in{'action'} $in{'vdev'}", $cmd);
}
elsif ($in{'cmd'} =~ "promote") {
	my $cmd = ($conf{'zfs_properties'}) ? "zfs promote $in{'zfs'}": undef;
	print ui_cmd("promote $in{'zfs'}", $cmd);
}
elsif ($in{'cmd'} =~ "scrub") {
	$in{'confirm'} = "yes";
	if ($in{'stop'}) { $in{'stop'} = "-s"; }
	my $cmd = ($conf{'pool_properties'}) ? "zpool scrub $in{'stop'} $in{'pool'}" : undef;
	print ui_cmd("scrub pool $in{'pool'}", $cmd);
}
elsif ($in{'cmd'} =~ "upgrade") {
	print "<p>".$text{'zpool_upgrade_msg'}."</p>";
	#$in{'confirm'} = "yes";
	my $cmd = ($conf{'pool_properties'}) ? "zpool upgrade $in{'pool'}" : undef;
	print ui_cmd("upgrade pool $in{'pool'}", $cmd);
}
elsif ($in{'cmd'} =~ "export") {
	#$in{'confirm'} = "yes";
	my $cmd = ($conf{'pool_properties'}) ? "zpool export $in{'pool'}" : undef;
	print ui_cmd("scrub pool $in{'pool'}", $cmd);
	...
	my $cmd = ($conf{'pool_properties'}) ? "zpool import".$dir." ".$in{'import'}: undef;
	print ui_cmd("import pool $in{'import'}", $cmd);
	@footer = ("index.cgi?mode=pools", $text{'index_return'});
}
elsif ($in{'cmd'} =~ "zfsact")  {
	my $cmd = ($conf{'zfs_properties'}) ? "zfs $in{'action'} $in{'zfs'}" : undef;
	print ui_cmd("$in{'action'} $in{'zfs'}", $cmd);
}
elsif ($in{'cmd'} =~ "zfsdestroy")  {
	my $cmd = ($conf{'zfs_destroy'}) ? "zfs destroy $in{'force'} $in{'zfs'}" : undef;
	if (!$in{'confirm'})
	{
	...
	my $cmd = ($conf{'snap_destroy'}) ? "zfs destroy $in{'force'} $in{'snapshot'}" : undef;
	if (!$in{'confirm'})
	{
